...
import { Trash2, Edit, Plus, Upload, X, Image as ImageIcon, Loader2, Search, User, Venue, GraduationCap, Calendar, Contact } from 'lucide-react';
...
export default function TeacherIndex({ teachers: allTeachers, institutions, can, flash, filters }: PageProps) {
  ...
  useEffect(() => {
    if (currentTeacher) {
      setData({
        ...currentTeacher,
        institutions: currentTeacher.institutions || [],
        ...
        <div className="flex flex-col gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              {filteredTeachers.length} enseignants enregistrés
            </p>
          </div>
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  {filteredTeachers.length} enseignants correspondant aux critères
                </CardDescription>
              </div>
            </div>
          </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover open={openInstitutions} onOpenChange={setOpenInstitutions}>
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                          role="combobox"
                          aria-expanded={openInstitutions}
                          className="w-full justify-between"
                        >
                          {data.institutions.length > 0
                            ? `${data.institutions.length} sélectionné(s)`
                            : "Sélectionnez des institutions..."}
                        </Button>
                      </PopoverTrigger>
                          ...
                          <CommandGroup className="max-h-48 overflow-y-auto">
                            {institutions.map((institution) => (
                              <CommandItem
                                key={institution.id}
                                onSelect={() => {
                                  const currentInstitutions = data.institutions.includes(institution.id)
                                    ? data.institutions.filter((i) => i !== institution.id)
                                    : [...data.institutions, institution.id]
                                  setData('institutions', currentInstitutions)
                                }}
                              >
                                <input
                                  type="checkbox"
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setData('academic_level', value)}
                    >
                      <SelectTrigger>
              ...
              <div className="space-y-4">
                ...
                >
                  {({ getRootProps, getInputProps }) => (
                    <div
                      {...getRootProps()}
                      className="border-2 border-dashed border-primary-100 rounded-xl p-6 text-center cursor-pointer bg-primary-50/50 hover:bg-primary-50 transition-colors"
                    >
                      <input {...getInputProps()} />
                      <div className="flex flex-col items-center gap-2 text-primary-600">
                        ...
                        <p className="text-sm text-primary-400">
                          Formats supportés: JPEG, PNG (max 1MB par fichier)
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  {(currentTeacher?.documents?.length ?? 0) > 0 && 
        ...
        <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Êtes-vous sûr de vouloir supprimer l'enseignant "{teacherToDelete?.name}" ? 
