...
class CinetPayService
{
    ...
    public function createPayment(?array $param)
    {
        ...
        $data = [
            ...
            "channels" => $channels,
            "metadata" => $param['metadata'] ?? "", // Add metadata support
            "alternative_currency" => "", // Optional
            'notify_url' => $param['notify_url'] ?? env('CINETPAY_NOTIFY_URL'),
            'return_url' => $param['return_url'] ?? env('CINETPAY_RETURN_URL'),
            'cancel_url' => $param['cancel_url'] ?? env('CINETPAY_CANCEL_URL'),
            'site_id' => env('CINETPAY_SITE_ID'),
            'apikey' => env('CINETPAY_API_KEY')
        ];

        Log::channel('cinetpay')->info('Initializing CinetPay Payment', ['payload' => $data]);

        $response = $this->client->post(env('CINETPAY_URL'), [
            'json' => $data,
            'verify' => false // Disable SSL verification if needed, or better, ensure certificates are set
        ]);

        $body = json_decode($response->getBody(), true);
        Log::channel('cinetpay')->info('CinetPay Initialization Response', ['response' => $body]);
